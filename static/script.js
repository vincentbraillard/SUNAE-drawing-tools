...
window.resetTextGrid = function() { document.querySelectorAll('.sunae-letter-box').forEach(input => input.value = ''); }
...
function getPerimeterTravel(p1, p2, round, w, h) {
    ...
    for(let i=0; i<=steps; i++) {
        ...
        pts.push(sanitizeCoordinates(px, py, round, w, h)); // Clampera le rectangle au bord parfait
    }
    pts.push(p2);
    return pts;
}
...
function setupWorkspace(tableName, round, w, h) {
    ...
    canvas.on('path:created', function(e) {
        ...
        for (let i = 0; i < e.path.path.length; i++) {
            ...
            if (cmd[0] === 'M' || cmd[0] === 'L') absPoints.push(sanitizeCoordinates(cmd[1], cmd[2], isRound, canvas.width, canvas.height));
            else if (cmd[0] === 'Q') absPoints.push(sanitizeCoordinates(cmd[3], cmd[4], isRound, canvas.width, canvas.height)); 
        }
    ...
    canvas.on('mouse:down', function(o) {
        ...
        tempLine = new fabric.Line([p.x, p.y, p.x, p.y], { strokeWidth: 3, fill: '#2980b9', stroke: '#2980b9', selectable: false, evented: false, isUserStroke: true, createdAt: Date.now() });
        canvas.add(tempLine);
    });
...
function setupSimulator() {
    ...
    let simOverlay = document.getElementById('sim-overlay') || document.createElement('div');
    if (!simOverlay.id) {
